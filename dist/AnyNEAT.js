...
function SAFE_HEAP_STORE(dest, value, bytes, isFloat) {
 ...
 if (runtimeInitialized) {
  ...
  assert(brk <= GROWABLE_HEAP_I8().length);
 }
...
function killThread(pthread_ptr) {
 ...
 SAFE_HEAP_STORE(pthread_ptr | 0, 0 | 0, 4);
...
var PThread = {
 ...
 allocateUnusedWorker: function() {
  var pthreadMainJs = locateFile("AnyNEAT.worker.js");
...
function establishStackSpace() {
 ...
 var stackTop = SAFE_HEAP_LOAD(pthread_ptr + 44 | 0, 4, 0) | 0;
 var stackSize = SAFE_HEAP_LOAD(pthread_ptr + 48 | 0, 4, 0) | 0;
...
function ___syscall_openat(dirfd, path, flags, varargs) {
 if (ENVIRONMENT_IS_PTHREAD) return _emscripten_proxy_to_main_thread_js(5, 1, dirfd, path, flags, varargs);
 SYSCALLS.varargs = varargs;
 try {
...
function _emscripten_get_screen_size(width, height) {
 if (ENVIRONMENT_IS_PTHREAD) return _emscripten_proxy_to_main_thread_js(31, 1, width, height);
...
function _emscripten_request_fullscreen_strategy(target, deferUntilInEventHandler, fullscreenStrategy) {
 ...
 var strategy = {
  ...
  canvasResizedCallbackTargetThread: SAFE_HEAP_LOAD(fullscreenStrategy + 20 | 0, 4, 0) | 0,
...
function _emscripten_set_window_title(title) {
 if (ENVIRONMENT_IS_PTHREAD) return _emscripten_proxy_to_main_thread_js(58, 1, title);
...
function _environ_get(__environ, environ_buf) {
 if (ENVIRONMENT_IS_PTHREAD) return _emscripten_proxy_to_main_thread_js(59, 1, __environ, environ_buf);
 var bufSize = 0;
...
function _environ_sizes_get(penviron_count, penviron_buf_size) {
 if (ENVIRONMENT_IS_PTHREAD) return _emscripten_proxy_to_main_thread_js(60, 1, penviron_count, penviron_buf_size);
...
function _fd_close(fd) {
 if (ENVIRONMENT_IS_PTHREAD) return _emscripten_proxy_to_main_thread_js(61, 1, fd);
 try {
  var stream = SYSCALLS.getStreamFromFD(fd);
  FS.close(stream);
  return 0;
...
function _fd_seek(fd, offset_low, offset_high, whence, newOffset) {
 if (ENVIRONMENT_IS_PTHREAD) return _emscripten_proxy_to_main_thread_js(63, 1, fd, offset_low, offset_high, whence, newOffset);
 try {
...
function _fd_write(fd, iov, iovcnt, pnum) {
 if (ENVIRONMENT_IS_PTHREAD) return _emscripten_proxy_to_main_thread_js(64, 1, fd, iov, iovcnt, pnum);
 try {
  var stream = SYSCALLS.getStreamFromFD(fd);
...
var __emscripten_allow_main_runtime_queued_calls = Module["__emscripten_allow_main_runtime_queued_calls"] = 110204;
